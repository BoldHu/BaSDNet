...
class Regression(nn.Module):
    """
    输入：
        x1: B x 256 x H/8 x W/8
        x2: B x 512 x H/16 x W/16
        x3: B x 1024 x H/32 x W/32
    输出：
        y: B x 1 x H/8 x W/8
    """
    def __init__(self):
        ...
        self.stage1 = nn.Sequential(
            nn.Conv2d(256, 128, 1, padding=0, dilation=1),#3 1
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True)
        )
        self.stage2 = nn.Sequential(
            nn.Conv2d(256, 128, 3, padding=2, dilation=2),#3 2
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True)
        )
        self.stage3 = nn.Sequential(
            nn.Conv2d(256, 128, 5, padding=6, dilation=3),#3 3
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True)
        )
...
class PyramidVisionTransformer(nn.Module):
    ...
    输入：
        x: B x 3 x H x W
    输出：
        x: B x embed_dims[-1]（最后一层特征）
    主要中间过程：
        每个stage输出：
            B x (H_i*W_i) x embed_dims[i]
        其中 H_i, W_i 依次为 H/4, H/8, H/16, H/32
    """
...
class CPVTV2(PyramidVisionTransformer):
    ...
    输入：
        x: B x 3 x H x W
    输出：
        outputs: list, 每个元素为 B x embed_dims[i] x H_i x W_i
        其中 i=0,1,2,3，H_i, W_i 依次为 H/4, H/8, H/16, H/32
    """
...
class PCPVT(CPVTV2):
    ...
    输出：
        outputs: list, 每个元素为 B x embed_dims[i] x H_i x W_i
        其中 i=0,1,2，H_i, W_i 依次为 H/4, H/8, H/16
    """
